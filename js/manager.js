...
ensureThemeApplied();
initAutoVersionCheck({ resourcePath: 'js/manager.js', storageKey: 'mts-version-token:manager' });
...
const state = {
    ...
    teamSalesOrders: [],
    teamSalesOrderItems: [],
    teamSalesOrderItemsByOrder: new Map(),
    mySalesOrders: [],
    mySalesOrderItems: [],
...
function setupTeamSalesTargetTabs() {
    const buttons = Array.from(document.querySelectorAll('#team-sales-target-tabs button'));
    const panels = Array.from(document.querySelectorAll('.team-sales-target-panel'));
    if (!buttons.length || !panels.length) return;
    initTabNavigation(buttons, panels, 'teamSalesTargetTab', 'accounts');
}
...
function initializeForms() {
    ...
    setupMyCaseForm();
    setupTeamSalesTargetTabs();
    setupTeamSalesTargetAccountFilters();
    setupTeamSalesTargetProductFilters();
    setupTeamSalesFilters();
    setupManagerSalesOrderForm();
    setupManagerSalesDashboardFilters();
...
function renderAll() {
    ...
    renderTeamSalesTargetSection();
    renderTeamSalesDataSection();
    renderManagerSalesDashboardSection();
    renderDashboard();
}
...
async function loadTeamCases() {
    ...
    state.teamCases = await loadAllPages(
        (offset, pageSize, page) => handleSupabase(
            ...
            `load team cases page ${page}`
        ),
        { pageSize: 1000 }
    );

    const teamCaseIds = state.teamCases.map((item) => item.id);
    state.teamCaseProducts = await loadAllByIdBatches(
        teamCaseIds,
        (idBatch, offset, pageSize, page, batchIndex) => handleSupabase(
            supabase
                ...
                .range(offset, offset + pageSize - 1),
            `load team case products batch ${batchIndex} page ${page}`
        ),
...
async function loadMyCases() {
    state.myCases = await loadAllPages(
        (offset, pageSize, page) => handleSupabase(
            supabase
                .from('v_case_details')
                .select('*')
                ...
                .order('id', { ascending: false })
                .range(offset, offset + pageSize - 1),
            `load manager cases page ${page}`
        ),
        { pageSize: 1000 }
    );

    const myCaseIds = state.myCases.map((item) => item.id);
    state.myCaseProducts = await loadAllByIdBatches(
        myCaseIds,
        (idBatch, offset, pageSize, page, batchIndex) => handleSupabase(
            supabase
                .from('case_products')
                ...
                .order('product_id', { ascending: true })
                .range(offset, offset + pageSize - 1),
            `load manager case products batch ${batchIndex} page ${page}`
        ),
        { idBatchSize: 500, pageSize: 1000 }
    );
...
function buildApprovalDatasets() {
    ...
    const orderPending = state.teamSalesOrders
        .filter((orderItem) => orderItem.status === APPROVAL_STATUS.PENDING_MANAGER)
        .map((orderItem) => ({
            id: orderItem.id,
            type: 'order',
            name: orderItem.order_code || `Order ${String(orderItem.id).slice(0, 6)}`,
            owner_id: orderItem.specialist_id,
            ownerName: ownerNameLookup.get(orderItem.specialist_id) || orderItem.specialist_name || 'Product Specialist',
            status: orderItem.status,
            created_at: orderItem.created_at,
            payload: orderItem
        }));

    state.approvals = [...pending, ...accountPending, ...casePending, ...orderPending];
    ...
    state.myApprovals = [
        ...
        ...state.mySalesOrders.map((orderItem) => ({
            id: orderItem.id,
            type: 'order',
...
async function handleTeamApproval(record, approve = true) {
    try {
        ...
        if (record.owner_id) {
            ...
            if (approve) {
                ...
                if (record.type === 'case') {
                    ...
                    message = `Sales order rejected by manager.${reason}`;
                } else {
        ...
        await Promise.all([loadTeamDoctors(), loadTeamAccounts(), loadTeamCases(), loadSalesExpansionData()]);
        buildApprovalDatasets();
        renderTeamDoctors({ refreshFilters: true });
        renderTeamAccounts({ refreshFilters: true });
        setupTeamFilters();
...
async function loadSalesExpansionData() {
    try {
        ...
        const teamMemberIds = new Set(state.teamMembers.map((member) => String(member.id)));
        const myId = String(state.session.employeeId || '');

        state.teamSalesOrders = allOrders.filter((order) => teamMemberIds.has(String(order.specialist_id)) || String(order.submitted_by_id) === myId);
        state.mySalesOrders = allOrders.filter((order) => String(order.specialist_id) === myId || String(order.submitted_by_id) === myId);

        const combinedOrderIds = distinct([...state.teamSalesOrders.map((order) => order.id), ...state.mySalesOrders.map((order) => order.id)]);
        const combinedItems = await loadAllByIdBatches(
            combinedOrderIds,
            (idBatch, offset, pageSize, page, batchIndex) => handleSupabase(
                supabase
                    .from('sales_order_items')
        ...
        const teamOrderIds = new Set(state.teamSalesOrders.map((order) => String(order.id)));
        const myOrderIds = new Set(state.mySalesOrders.map((order) => String(order.id)));
        state.teamSalesOrderItems = (combinedItems || []).filter((item) => teamOrderIds.has(String(item.sales_order_id)));
        state.mySalesOrderItems = (combinedItems || []).filter((item) => myOrderIds.has(String(item.sales_order_id)));
...
function setupManagerSalesOrderButton() {
}

function setupManagerSalesOrderForm() {
    const container = document.querySelector('#manager-sales-order-form .row');
    if (!container) return;
    container.innerHTML = `
        <div class="col-12">
            <div id="manager-sales-order-feedback" class="alert-feedback d-none"></div>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-12">
            ...
            <div id="manager-sales-order-products" class="d-grid gap-3"></div>
            <div class="d-flex justify-content-between mt-2">
                <button type="button" class="btn btn-outline-ghost" id="manager-sales-order-add-product">Add Product</button>
                <button type="button" class="btn btn-outline-ghost" id="manager-sales-order-remove-product">Remove Product</button>
            </div>
        </div>
        <div class="col-12">
        ...
        <div class="col-12 d-flex justify-content-end gap-2">
            <button type="button" class="btn btn-outline-ghost" id="manager-sales-order-reset">Reset</button>
            <button type="submit" class="btn btn-gradient">Submit Order</button>
        </div>
    `;

    const form = document.getElementById('manager-sales-order-form');
    if (!form) return;
    const feedback = document.getElementById('manager-sales-order-feedback');
    ...
    form.addEventListener('submit', handleManagerSalesOrderSubmit);
    form.addEventListener('mts:form-open', () => resetForm());
    form.addEventListener('mts:form-close', () => hideAlert(feedback));
    form.querySelector('#manager-sales-order-reset')?.addEventListener('click', resetForm);
    form.querySelector('#manager-sales-order-add-product')?.addEventListener('click', () => {
        if (state.managerSalesOrderFormRows < MAX_PRODUCTS_PER_ORDER) {
            state.managerSalesOrderFormRows += 1;
            renderManagerSalesOrderProductRows();
        }
    });
    form.querySelector('#manager-sales-order-remove-product')?.addEventListener('click', () => {
        if (state.managerSalesOrderFormRows > 1) {
            state.managerSalesOrderFormRows -= 1;
            renderManagerSalesOrderProductRows();
        }
    });
    specialistSelect?.addEventListener('change', () => {
        const selected = state.specialists.find((sp) => String(sp.id) === String(specialistSelect.value));
        const line = state.lines.find((item) => String(item.id) === String(selected?.line_id));
        form.querySelector('input[name="line_id"]').value = selected?.line_id || '';
        form.querySelector('input[name="line_name"]').value = line?.name || '';
...
function populateManagerSalesOrderSpecialists(selectedId = '') {
    const form = document.getElementById('manager-sales-order-form');
    if (!form) return;
    const specialistSelect = form.querySelector('select[name="specialist_id"]');
    if (!specialistSelect) return;
    const specialists = [...state.specialists].sort((a, b) => `${a.first_name || ''} ${a.last_name || ''}`.localeCompare(`${b.first_name || ''} ${b.last_name || ''}`));
    specialistSelect.innerHTML = `
        <option value="">Select product specialist</option>
        ${specialists.map((sp) => `<option value="${sp.id}">${escapeOptionText(`${sp.first_name || ''} ${sp.last_name || ''}`.trim())}</option>`).join('')}
    `;
    if (selectedId && specialists.some((sp) => String(sp.id) === String(selectedId))) {
        specialistSelect.value = String(selectedId);
    } else if (specialists.length) {
        specialistSelect.value = String(specialists[0].id);
    }
    makeSelectSearchable?.(specialistSelect);
    const selected = specialists.find((sp) => String(sp.id) === String(specialistSelect.value));
    const line = state.lines.find((item) => String(item.id) === String(selected?.line_id));
    form.querySelector('input[name="line_id"]').value = selected?.line_id || '';
    form.querySelector('input[name="line_name"]').value = line?.name || '';
    populateManagerSalesOrderAccounts();
...
function populateManagerSalesOrderAccounts(selectedId = '') {
    const form = document.getElementById('manager-sales-order-form');
    if (!form) return;
    ...
    let accounts = state.accounts.filter((acc) => acc.status === APPROVAL_STATUS.APPROVED && (!lineId || String(acc.line_id) === String(lineId)));
...
function getManagerSalesOrderProducts() {
    const form = document.getElementById('manager-sales-order-form');
    if (!form) return [];
    const lineId = form.querySelector('input[name="line_id"]')?.value || '';
...
function renderManagerSalesOrderProductRows() {
    const form = document.getElementById('manager-sales-order-form');
    const container = document.getElementById('manager-sales-order-products');
    if (!form || !container) return;
    const orderType = form.querySelector('select[name="order_type"]')?.value || 'company';

    const preserved = new Map();
    container.querySelectorAll('[data-manager-sales-row]').forEach((rowEl) => {
        const row = Number(rowEl.dataset.managerSalesRow);
        preserved.set(row, {
            companyId: rowEl.querySelector(`[name="m_order_company_${row}"]`)?.value || '',
            productId: rowEl.querySelector(`[name="m_order_product_${row}"]`)?.value || '',
            units: rowEl.querySelector(`[name="m_order_units_${row}"]`)?.value || '0',
            price: rowEl.querySelector(`[name="m_order_price_${row}"]`)?.value || ''
        });
    });

    const products = getManagerSalesOrderProducts();
    ...
    container.innerHTML = '';
    for (let row = 1; row <= state.managerSalesOrderFormRows; row += 1) {
        const saved = preserved.get(row) || {};
        const rowEl = document.createElement('div');
        rowEl.className = 'row g-3 align-items-end';
        rowEl.dataset.managerSalesRow = String(row);
        rowEl.innerHTML = `
            <div class="col-md-3">
                <label class="form-label">Company ${row}</label>
                <select class="form-select" name="m_order_company_${row}">
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="m_order_product_${row}">
                    <option value="">Select product</option>
                </select>
            </div>
            <div class="col-md-2">
                <label class="form-label">Units</label>
                <input type="number" class="form-control" name="m_order_units_${row}" min="1" value="${escapeOptionText(saved.units || '1')}">
            </div>
            <div class="col-md-3">
                <label class="form-label">Unit Price</label>
                <input type="number" class="form-control" name="m_order_price_${row}" min="0" step="0.01" placeholder="${orderType === 'competitor' ? 'Required' : 'Auto from target'}" value="${escapeOptionText(saved.price || '')}" ${orderType === 'competitor' ? 'required' : ''}>
            </div>
        `;
        container.appendChild(rowEl);

        const companySelect = rowEl.querySelector(`[name="m_order_company_${row}"]`);
        const productSelect = rowEl.querySelector(`[name="m_order_product_${row}"]`);
        const unitsInput = rowEl.querySelector(`[name="m_order_units_${row}"]`);
        const priceInput = rowEl.querySelector(`[name="m_order_price_${row}"]`);

        if (companySelect && saved.companyId) companySelect.value = saved.companyId;
        populateManagerSalesOrderProductOptions(row, saved.companyId || '', saved.productId || '');
        ...
        companySelect?.addEventListener('change', () => {
            populateManagerSalesOrderProductOptions(row, companySelect.value, '');
...
function populateManagerSalesOrderProductOptions(rowIndex, companyId, selectedProductId = '') {
    const form = document.getElementById('manager-sales-order-form');
    if (!form) return;
    const orderType = form.querySelector('select[name="order_type"]')?.value || 'company';
    const productSelect = form.querySelector(`[name="m_order_product_${rowIndex}"]`);
...
async function handleManagerSalesOrderSubmit(event) {
    event.preventDefault();
    ...
    if (!payload.specialist_id || !payload.account_id || !payload.order_date || !payload.order_type || !payload.line_id) {
        showAlert(feedback, 'Product Specialist, Account, Order Date, and Order Type are required.');
        return;
    }

    const specialist = state.specialists.find((sp) => String(sp.id) === String(payload.specialist_id));
    if (!specialist) {
        showAlert(feedback, 'Selected product specialist is not in your team.');
        return;
    }

    const items = [];
    for (let row = 1; row <= state.managerSalesOrderFormRows; row += 1) {
        const productId = payload[`m_order_product_${row}`];
        const unitsRaw = payload[`m_order_units_${row}`];
        const priceRaw = payload[`m_order_price_${row}`];
...
function setupTeamSalesFilters() {
    const container = document.getElementById('team-sales-filters');
    if (!container) return;
    const previousSelections = {
        specialist: container.querySelector('#team-sales-filter-specialist')?.value || '',
        accountType: container.querySelector('#team-sales-filter-account-type')?.value || '',
        orderType: container.querySelector('#team-sales-filter-order-type')?.value || '',
        companyCompany: container.querySelector('#team-sales-filter-company-company')?.value || '',
        companyCategory: container.querySelector('#team-sales-filter-company-category')?.value || '',
        companySubCategory: container.querySelector('#team-sales-filter-company-sub-category')?.value || '',
        companyProduct: container.querySelector('#team-sales-filter-company-product')?.value || '',
        competitorCompany: container.querySelector('#team-sales-filter-competitor-company')?.value || '',
        competitorCategory: container.querySelector('#team-sales-filter-competitor-category')?.value || '',
        competitorSubCategory: container.querySelector('#team-sales-filter-competitor-sub-category')?.value || '',
        competitorProduct: container.querySelector('#team-sales-filter-competitor-product')?.value || '',
        accountName: container.querySelector('#team-sales-filter-account-name')?.value || ''
    };
    const specialists = state.specialists || [];
    const { company, competitor } = collectDualRowFilterOptions(getEnrichedTeamSalesItems(state.teamSalesOrderItems), state.products);
    container.innerHTML = `
        <div class="filters-row">
            <select class="form-select" id="team-sales-filter-specialist">
                <option value="">All Specialists</option>
                ${specialists.map((item) => `<option value="${item.id}">${item.first_name} ${item.last_name}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-account-type">
                <option value="">All Account Types</option>
                ${ACCOUNT_TYPES.map((type) => `<option value="${type}">${type}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-order-type">
                <option value="">All Order Types</option>
                <option value="company">Company Order</option>
                <option value="competitor">Competitor Order</option>
            </select>
            <input type="text" class="form-control" id="team-sales-filter-account-name" placeholder="Search Account Name...">
        </div>
        <div class="filters-row">
            <select class="form-select" id="team-sales-filter-company-company">
                <option value="">All Companies</option>
                ${company.companies.map((item) => `<option value="${item}">${item}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-company-category">
                <option value="">All Categories</option>
                ${company.categories.map((item) => `<option value="${item}">${item}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-company-sub-category">
                <option value="">All Sub Categories</option>
                ${company.subCategories.map((item) => `<option value="${escapeOptionText(item)}">${escapeOptionText(item)}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-company-product">
                <option value="">All Products</option>
                ${company.productOptions.map((option) => `<option value="${option.value}">${escapeOptionText(option.label)}</option>`).join('')}
            </select>
        </div>
        <div class="filters-row">
            <select class="form-select" id="team-sales-filter-competitor-company">
                <option value="">All Companies</option>
                ${competitor.companies.map((item) => `<option value="${item}">${item}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-competitor-category">
                <option value="">All Categories</option>
                ${competitor.categories.map((item) => `<option value="${item}">${item}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-competitor-sub-category">
                <option value="">All Sub Categories</option>
                ${competitor.subCategories.map((item) => `<option value="${escapeOptionText(item)}">${escapeOptionText(item)}</option>`).join('')}
            </select>
            <select class="form-select" id="team-sales-filter-competitor-product">
                <option value="">All Products</option>
                ${competitor.productOptions.map((option) => `<option value="${option.value}">${escapeOptionText(option.label)}</option>`).join('')}
            </select>
        </div>
    `;
    const rerender = () => renderTeamSalesDataSection();
    container.querySelectorAll('select').forEach((el) => el.addEventListener('change', rerender));
    container.querySelector('#team-sales-filter-account-name')?.addEventListener('input', rerender);
    const setSelectValue = (selector, value) => {
    ...
    setSelectValue('#team-sales-filter-specialist', previousSelections.specialist);
    setSelectValue('#team-sales-filter-account-type', previousSelections.accountType);
    setSelectValue('#team-sales-filter-order-type', previousSelections.orderType);
    setSelectValue('#team-sales-filter-company-company', previousSelections.companyCompany);
    setSelectValue('#team-sales-filter-company-category', previousSelections.companyCategory);
    setSelectValue('#team-sales-filter-company-sub-category', previousSelections.companySubCategory);
    setSelectValue('#team-sales-filter-company-product', previousSelections.companyProduct);
    setSelectValue('#team-sales-filter-competitor-company', previousSelections.competitorCompany);
    setSelectValue('#team-sales-filter-competitor-category', previousSelections.competitorCategory);
    setSelectValue('#team-sales-filter-competitor-sub-category', previousSelections.competitorSubCategory);
    setSelectValue('#team-sales-filter-competitor-product', previousSelections.competitorProduct);
    const accountNameInput = container.querySelector('#team-sales-filter-account-name');
    if (accountNameInput) accountNameInput.value = previousSelections.accountName;
}

function getFilteredTeamSalesOrders() {
    const specialist = document.getElementById('team-sales-filter-specialist')?.value || '';
    const accountType = document.getElementById('team-sales-filter-account-type')?.value || '';
    const orderType = document.getElementById('team-sales-filter-order-type')?.value || '';
    const companyCompany = document.getElementById('team-sales-filter-company-company')?.value || '';
    const companyCategory = document.getElementById('team-sales-filter-company-category')?.value || '';
    const companySubCategory = document.getElementById('team-sales-filter-company-sub-category')?.value || '';
    const companyProduct = document.getElementById('team-sales-filter-company-product')?.value || '';
    const competitorCompany = document.getElementById('team-sales-filter-competitor-company')?.value || '';
    const competitorCategory = document.getElementById('team-sales-filter-competitor-category')?.value || '';
    const competitorSubCategory = document.getElementById('team-sales-filter-competitor-sub-category')?.value || '';
    const competitorProduct = document.getElementById('team-sales-filter-competitor-product')?.value || '';
    const accountName = (document.getElementById('team-sales-filter-account-name')?.value || '').trim().toLowerCase();
    ...
    return state.teamSalesOrders.filter((order) => {
        ...
        if (hasCompanyFilters || hasCompetitorFilters) {
            const items = getEnrichedTeamSalesItems(state.teamSalesOrderItemsByOrder.get(order.id) || []);
            if (hasCompanyFilters) {
...
function getEnrichedTeamSalesItems(items = []) {
...
function getTeamSalesMetricScopeFromFilters() {
    const specialist = document.getElementById('team-sales-filter-specialist')?.value || '';
    const accountType = document.getElementById('team-sales-filter-account-type')?.value || '';
    const accountName = (document.getElementById('team-sales-filter-account-name')?.value || '').trim().toLowerCase();
    const teamIds = new Set(state.teamMembers.map((member) => String(member.id)));

    let scopedAccounts = state.accounts.filter((acc) => acc.status === APPROVAL_STATUS.APPROVED && teamIds.has(String(acc.owner_employee_id)));
    ...
    const relevantProductTargets = state.salesProductTargets.filter((target) =>
        specialistIds.has(String(target.specialist_id || ''))
    );

    return { relevantAccountTargets, relevantProductTargets };
}

function computeTeamSalesSummaryMetrics(orders = [], scope = null) {
    ...
    const relevantAccountTargets = scope?.relevantAccountTargets
        || state.salesAccountTargets.filter((target) => teamIds.has(String(target.specialist_id)));
    const relevantProductTargets = scope?.relevantProductTargets
        || state.salesProductTargets.filter((target) => teamIds.has(String(target.specialist_id)));

    const scopedOrderIds = new Set(orders.map((order) => String(order.id)));
    const scopedItems = getEnrichedTeamSalesItems(state.teamSalesOrderItems.filter((item) => scopedOrderIds.has(String(item.sales_order_id))));
...
function buildTeamSalesRow(order) {
    const items = state.teamSalesOrderItemsByOrder.get(order.id) || [];
    const row = {
        id: order.id,
...
function buildTeamSalesColumns() {
    const columns = [
...
function renderTeamSalesDataSection() {
    const orders = getFilteredTeamSalesOrders();
    const scope = getTeamSalesMetricScopeFromFilters();
    const stats = document.getElementById('team-sales-stat-cards');
    if (stats) {
    ...
    state.tables.teamSales = createTable('team-sales-table', buildTeamSalesColumns(), orders.map((o) => buildTeamSalesRow(o)), {
        height: 520,
...
function setupManagerSalesDashboardFilters() {
    const container = document.getElementById('manager-sales-dashboard-filters');
    if (!container) return;
    container.innerHTML = '<div class="text-secondary">Sales dashboard filters placeholder.</div>';
}

function renderManagerSalesDashboardSection() {
    const stats = document.getElementById('manager-sales-dashboard-stat-cards');
    if (!stats) return;
    const orders = getFilteredTeamSalesOrders();
...
function populateTeamSalesOrderReview(id, payload = {}, showActions = false) {
    const orderRecord = state.teamSalesOrders.find((item) => item.id === id) || payload || {};
    const products = state.teamSalesOrderItemsByOrder.get(id) || [];
    ...
    showManagerReviewModal(`Review Order ${orderRecord.order_code || ''}`, content, { showCaseActions: showActions, caseRecord: recordForApproval });
}

async function openManagerSalesOrderWizard() {
    return;
}
...
function reviewTeamApproval(record) {
    if (record.type === 'doctor') {
        ...
        populateTeamSalesOrderReview(record.id, record.payload, true);
    }
}
